...
class NotificationHandlerFactory(private val context: Context) {
    ...
    fun getHandler(type: NotificationType): NotificationHandler {
        return when (type) {
            ...
            NotificationType.EXPENSE_UPDATED -> ExpenseUpdatedHandler(context)
            NotificationType.EXPENSE_DELETED -> ExpenseDeletedHandler(context)
            NotificationType.MEMBER_ADDED -> MemberAddedHandler(context)
            NotificationType.MEMBER_REMOVED -> MemberRemovedHandler(context)
            NotificationType.CASH_WITHDRAWAL -> CashWithdrawalHandler(context)
            NotificationType.CONTRIBUTION_ADDED -> ContributionAddedHandler(context)
