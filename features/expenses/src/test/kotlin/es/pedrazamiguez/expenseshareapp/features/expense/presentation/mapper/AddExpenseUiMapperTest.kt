...
class AddExpenseUiMapperTest {
    ...
    fun setup() {
        ...
        mapper = AddExpenseUiMapper(localeProvider, resourceProvider, ExpenseCalculatorService())
    }
    ...
    @Nested
    inner class MapToDomainIncludedAddOnAdjustment {
        ...
        fun `EXACT INCLUDED tip subtracts from sourceAmount and groupAmount`() {
            ...
            assertTrue(result.isSuccess)
            val expense = result.getOrThrow()
            assertEquals(6210L, expense.sourceAmount)
            assertEquals(6210L, expense.groupAmount)
            // Add-on groupAmountCents unchanged for EXACT
            assertEquals(621L, expense.addOns[0].groupAmountCents)
        }

        @Test
        fun `PERCENTAGE INCLUDED tip extracts base and recomputes add-on amount`() {
            // 68.31 EUR total with 10% INCLUDED tip
            // Base: 68.31 / 1.10 = 62.10 EUR (6210 cents)
            // Recomputed tip: 6210 * 10% = 621 cents
            val addOn = AddOnUiModel(
                id = "tip-1",
                type = AddOnType.TIP,
                mode = AddOnMode.INCLUDED,
                valueType = AddOnValueType.PERCENTAGE,
                amountInput = "10",
                resolvedAmountCents = 683, // 10% of 6831 (original, pre-adjustment)
            ...
            assertEquals(6210L, expense.sourceAmount)
            assertEquals(6210L, expense.groupAmount)
            // Recomputed: 6210 * 10 / 100 = 621
            assertEquals(621L, expense.addOns[0].groupAmountCents)
        }

        @Test
        fun `splits are rescaled proportionally for INCLUDED add-ons`() {
            // 100.00 EUR total with 10 EUR EXACT INCLUDED tip, 2 equal splits
            // Base: 100 - 10 = 90 EUR (9000 cents)
            // Splits should rescale from 5000 each to 4500 each
            val addOn = AddOnUiModel(
                id = "tip-1",
                type = AddOnType.TIP,
                mode = AddOnMode.INCLUDED,
            ...
            val split1 = es.pedrazamiguez.expenseshareapp.features.expense.presentation.model.SplitUiModel(
                userId = "user-1",
                displayName = "User 1",
                amountCents = 5000
            )
            val split2 = es.pedrazamiguez.expenseshareapp.features.expense.presentation.model.SplitUiModel(
                userId = "user-2",
                displayName = "User 2",
                amountCents = 5000
            )
            ...
            val state = AddExpenseUiState(
                ...
                selectedPaymentMethod = cashPaymentMethod,
                addOns = kotlinx.collections.immutable.persistentListOf(addOn),
                splits = kotlinx.collections.immutable.persistentListOf(split1, split2)
            )
            ...
            assertTrue(result.isSuccess)
            val expense = result.getOrThrow()
            assertEquals(9000L, expense.sourceAmount)
            // Splits rescaled: 4500 each, sum = 9000
            assertEquals(9000L, expense.splits.sumOf { it.amountCents })
            assertEquals(4500L, expense.splits[0].amountCents)
            assertEquals(4500L, expense.splits[1].amountCents)
        }

        @Test
        fun `no adjustment when no INCLUDED add-ons present`() {
            val addOn = AddOnUiModel(
                id = "fee-1",
                type = AddOnType.FEE,
                mode = AddOnMode.ON_TOP,
            ...
            assertTrue(result.isSuccess)
            val expense = result.getOrThrow()
            // No INCLUDED → amounts unchanged
            assertEquals(5000L, expense.sourceAmount)
            assertEquals(5000L, expense.groupAmount)
            assertEquals(500L, expense.addOns[0].groupAmountCents)
        }

        @Test
        fun `INCLUDED DISCOUNT is excluded from base cost adjustment`() {
            // An INCLUDED DISCOUNT should not trigger base cost extraction
            val addOn = AddOnUiModel(
                id = "disc-1",
            ...
            assertTrue(result.isSuccess)
            val expense = result.getOrThrow()
            // DISCOUNT is excluded from INCLUDED adjustment → amounts unchanged
            assertEquals(5000L, expense.sourceAmount)
            assertEquals(5000L, expense.groupAmount)
        }
    }
